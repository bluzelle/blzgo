...
func main() {
	...
	if err := ctx.Create(key1, "value", bluzelle.GetTestGasInfo(), nil); err != nil {
		log.Fatalf("%s", err)
	}
	if err := ctx.Create(key2, "value", bluzelle.GetTestGasInfo(), nil); err != nil {
		log.Fatalf("%s", err)
	}
	...
	if err := ctx.MultiUpdate(keyValues, bluzelle.GetTestGasInfo()); err != nil {
		log.Fatalf("%s", err)
	} else {
		log.Infof("done")
	}
